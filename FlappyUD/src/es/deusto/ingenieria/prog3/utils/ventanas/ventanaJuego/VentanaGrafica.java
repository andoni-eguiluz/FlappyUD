...
//						pAreaJuego.repaint( oj.getX(), oj.getY(), oj.getAnchuraObjeto(), oj.getAlturaObjeto() );
	...
	public boolean quitaDibujoEnPosicion( Point pos ) {
		...
		dibujosEnPantalla.remove( dibujo );
		layeredPane.repaint();
		return true;
	}
	...
	public boolean traeDibujoAlFrente( Point pos ) {
		Dibujo dibujo = dibujoDeEstePunto( pos );
		if (dibujo==null) return false;
		dibujosEnPantalla.remove( dibujo ); // Quita de donde esté
		dibujosEnPantalla.add( dibujo );  // Añade al frente
		layeredPane.repaint();
		return true;
	}
	 ...
	 */
	public void borraDibujoRect( int xCentro, int yCentro, int ancho, int alto, Color color, int anchuraDibujo ) {
		for (Dibujo d : dibujosEnPantalla) {
